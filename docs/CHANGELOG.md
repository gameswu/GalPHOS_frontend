...
## 版本 v1.3.4 (2025-07-07)

### 🔧 考试文件上传API修复
- **修复文件类型区分问题**:
  - 重新设计考试文件上传API：`POST /api/admin/exams/{examId}/files`
  - 添加必需的`type`参数：question（题目）、answer（答案）、answer_sheet（答题卡）
  - 后端现在可以正确区分和处理不同类型的考试文件
  - 完善了文件管理接口：获取、下载、删除、替换文件
  - 提供了详细的前端集成示例和使用说明
...
- **教练所属学生账号特性明确**:
  ...
  - 简化了教练管理学生的操作流程
  - 所有类型的用户都统一使用用户名作为唯一标识，不再需要姓名字段
